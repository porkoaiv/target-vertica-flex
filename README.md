...
Create tests within the `target_vertica/tests` subfolder and
  then run:
...
The below table shows how this tap will map between jsonschema datatypes and Vertica datatypes.
...
If a column has multiple jsonschema types, the following order is using to order Vertica types, from highest priority to lowest priority.
- BYTEA
- ARRAY(JSONB)
- JSONB
- TEXT
...
This target can detect content encoding clues in the schema to determine how to store the data in the vertica in a more efficient way.
